...
deals={}
for r in J["rows"]:
    if r[C["st"]]=="NY" and r[C["bbl"]]: deals.setdefault(int(r[C["bbl"]]),[]).append(r)
...
for r in D.itertuples():
    ...
    rec={"bbl":bbl,"addr":str(r.address or "").title(),"zip":str(r.zipcode or "")[:5],"boro":boro,"cd":r.cd,"lat":round(r.latitude,5),"lng":round(r.longitude,5),
         "bc":r.bldgclass,"type":AC.get(str(r.bldgclass)[:1],"Other"),"tc":tc,"lu":r.landuse,
         "owner":str(r.owner or r.ownername or "").title()[:100],"units":iv(r.unitsres),"unitsT":iv(r.unitstotal),"sf":iv(sf),"stories":fv(r.numfloors,1),"lot":iv(r.lotarea),"lotF":fv(r.lotfront,1),"lotD":fv(r.lotdepth,1),"bF":fv(r.bldgfront,1),"bD":fv(r.bldgdepth,1),
         "yb":iv(r.yearbuilt),"alt":iv(r.yearalter1),"zone":str(r.zonedist1 or ""),"far":fv(r.builtfar),"maxFar":fv(max(r.residfar or 0,r.commfar or 0)),"resA":iv(r.resarea),"comA":iv(r.comarea),"offA":iv(r.officearea),"retA":iv(r.retailarea),
...
for b,L2 in out.items():
    json.dump(L2,open(f"../repo/site/props/NY_{b.replace(' ','_')}.json","w"),separators=(",",":")); print(b,len(L2),os.path.getsize(f"../repo/site/props/NY_{b.replace(' ','_')}.json")//1024//1024,"MB")
