...
for r in s.itertuples():
    ...
    rows.append([r.dt.strftime("%Y-%m-%d"),cty+" County",mun,str(r.addr or "").title() or "Address not listed",a,r.pc,units,None,int(r.price),1+sum(1 for x in [r.etc] if x=="X"),round(float(lat),5),round(float(lng),5),"",owner[:150],conf+nuflag,"",mail[:120],"","",int(r.yb) if r.yb.isdigit() and int(r.yb)>1600 else None,"",int(float(r.CALC_ACRE)*43560) if pd.notna(r.CALC_ACRE) and r.CALC_ACRE>0 else None,"","","","","NJ"])
