...
while True:
    try: j=requests.post(U,data={"where":"1=1","outFields":"PIN,LAT,LON,ADDR_FULL,CTYNAME,ZIP5","returnGeometry":"false","resultOffset":off,"resultRecordCount":1000,"orderByFields":"OBJECTID","f":"json"},timeout=180).json()
    ...
    off+=len(fs)
    if off%50000<1000: print("geo",off,flush=True)
    if not fs: break
