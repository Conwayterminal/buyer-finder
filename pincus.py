...
for p in range(1,pages+1):
    ...
    for link,title,body in items:
        ...
        D["rows"].append([dt,boro,nbhd,addr or nbhd+" (address pending)",asset,bc,int(units.group(1)) if units else None,int(sf.group(1).replace(",","")) if sf else None,price,1,lat,lng,"",buyer,"PincusCo report (true owner as reported)",seller,"","","",None,"",None,link,None,None,None,"NY",None])
...
for st in ["NY"]:  # daily job owns only the NYC market file; state jobs own theirs
